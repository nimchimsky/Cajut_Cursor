...
OUT=ROOT/'qa_v3_workbench'; OUT.mkdir(parents=True,exist_ok=True)
...
for obj in list(bpy.data.objects):
    name=obj.name
    if name.startswith('Raig_2_') or name.startswith('Broquet_2_'):
        bpy.data.objects.remove(obj,do_unlink=True)
        continue
    if name.startswith('Edifici_context_'):
...
bpy.ops.wm.save_as_mainfile(filepath=str(ROOT/'Font_del_Centenari_Tarragona_QA_v3.blend'))
